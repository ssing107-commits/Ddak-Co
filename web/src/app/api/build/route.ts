...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!files.some((f) => f.path === "package.json")) {
      return NextResponse.json(
        { error: "package.json이 생성되지 않았습니다." },
        { status: 502 }
      );
    }
  } catch (e) {
  ...
  try {
    ...
    const up = await waitForPreviewUrl(previewUrl);
    if (!up) {
      return NextResponse.json(
        {
          error:
            "개발 서버가 제한 시간 내에 응답하지 않았습니다. 잠시 후 다시 시도해 주세요.",
          previewUrl,
